...
class Availability_performance():
    ...
    def calculate_normalized_availability_performance(data, potential_linktime):
        max_time = np.max(potential_linktime, axis=1)
        normalized_availability_performance = data / max_time[:, np.newaxis]

        return normalized_availability_performance
