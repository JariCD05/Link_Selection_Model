...
class Throughput_performance:
    ...
    def calculate_normalized_throughput_performance(data, potential_linktime):
        max_time = np.max(potential_linktime, axis=1)
        normalized_throughput_performance = data / max_time[:, np.newaxis]

        return normalized_throughput_performance
