...
class link_propagation():
    ...
    # Tijmen, what is the thing here with also having to providng index_elevation as an input
        ...
        # the geometricla output for all applicable satellites should be used, for now the lenght of ranges does not match the length of time as the zero values are not stored
        def calculate_latency(self, applicable_output, num_satellites):
            link_geometry.geometrical_outputs()
            range_index = 0
            ranges = applicable_output['ranges'] 
...
#            #the goal within the availability perforamnce parameter is to check if the power received + penalty is larger than BER induced power threshold
#            #for each applicable satellite, check if P_RX 
##            link = link_budget()
#            self.link_availability = np.zeros((num_satellites, len(time)))
#            for sat_index in range:
                

    # Jari, have a look at the mask.astype steup and where it comes from
        def availability_old(self, time_links):
            # Availability
        ...
        def throughput_old(self, throughput, find_link_margin, indices, index_elevation, SNR_penalty):
            LCT = terminal_properties()
            link = link_budget()
...
link_propagation_test = link_propagation()
link_geometry = link_geometry()
link_geometry.propagate(time=t_macro, step_size_AC=step_size_AC, step_size_SC=step_size_SC,
                        aircraft_filename=aircraft_filename_load, step_size_analysis=False, verification_cons=False)
...
Latency_propagation = link_propagation_test.calculate_latency(applicable_output=applicable_output, num_satellites= num_satellites)
